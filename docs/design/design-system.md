...
- `/` 是仪表盘：孕周/宝宝状态头、五项快捷入口、紧凑准备进度和家庭成员引导。
- `/tools` 是工具 hub，一级入口为孕期成长记 `/growth`、准备出发 `/departure` 和清单设置 `/settings/checklist`。
- 导航归属（`lib/navigation.ts` ownedRoutes）：清单只拥有 `/checklist`；工具拥有 `/tools`、`/growth`、`/departure`；我的拥有 `/settings`、`/privacy`、`/support`。
...
视觉语言延续「暖奶油风」：暖米白底、纯白大圆角无边框卡片、粉/杏/薄荷马卡龙大色块、陶土珊瑚红小面积主动作和暖棕分层阴影。
...
- 清单首页顶部是粉/杏/薄荷马卡龙进度 hero（完成度进度环 + 待买/待装/已装统计），下方按证件包、产房包、病房包 · 妈妈、病房包 · 宝宝组成「住院分包」网格，其余分组归入「后续准备」；点击进入独立分类页，分类页物品卡极窄屏单列、`xs`（360px）起双列。
- 行的首要动作是状态推进；默认只展示名称、数量、一句话说明、状态和缩略图。
- 编辑、标记不需要、删除与完整说明放在行详情。
...
- 顶部说明清单默认只保存在当前设备，并摘要显示家庭同步与本机恢复点状态。
- 不提供手工复制或粘贴备份文本；用户只面对本机恢复点、JSON 备份、照片迁移包与家庭同步。
- WebDAV 备份通道已下线；旧客户端保存在本机的 WebDAV 地址与明文密码会在启动时清除。
- 本地恢复快照与清空重建分开呈现；清空是危险操作，必须二次确认并在按钮附近反馈结果。
- 本地快照与 JSON 备份包含清单与成长记的便携数据，不包含物品照片。

## 照片（已下线）

- 物品照片的拍摄与展示已下线，清单恢复使用统一插画。
- 此前拍摄的照片仍保存在本机 IndexedDB；「备份与恢复」保留照片包导出/导入作为迁移窗口，不进入本机恢复点。
- 清空数据时会一并删除照片库。
...
- Android：`android/` 是直接加载 `https://dadkit.505f.com` 的轻量 WebView 壳，不再打包 `assets/www`。线上页面导航采用 network-first，成功访问后由 Service Worker 提供离线缓存；当前会话不因部署而刷新，下次启动获取最新页面。原生数据迁移、主题与文件选择桥继续保留；versionCode 27 起外壳不再内置更新桥与安装权限，外壳更新统一引导到 GitHub Releases 外链，旧 APK 中的 `DadKitAndroidUpdate` 桥闲置但保持兼容。「我的 → 关于 DadKit」分别显示页面版本与 Android 外壳 versionCode，并提供「打开 GitHub Releases」入口。发布校验走 `scripts/validate-android-release.mjs`，确认 APK 不含网页包。
