import type { TemplateChecklistItem } from "@/lib/types";

type ItemOptions = Partial<
  Omit<
    TemplateChecklistItem,
    | "id"
    | "name"
    | "category"
    | "priority"
    | "source"
    | "sourceLabel"
    | "editable"
    | "timing"
  >
>;

function templateItem(
  id: string,
  name: TemplateChecklistItem["name"],
  category: TemplateChecklistItem["category"],
  priority: TemplateChecklistItem["priority"],
  timing: TemplateChecklistItem["timing"],
  options: ItemOptions = {},
): TemplateChecklistItem {
  return {
    id,
    name,
    category,
    priority,
    source: "general",
    sourceLabel: "通用模板",
    editable: true,
    removable: true,
    timing,
    ...options,
  };
}

export const generalTemplate: TemplateChecklistItem[] = [
  templateItem("general-doc-id", "身份证件", "documents", "must", "pack_now", {
    quantity: "父母双方原件各 1 份，复印件按医院要求",
    note: "用于入院登记及后续材料核验；建议父母双方原件分别装袋，复印件数量先向医院确认。",
    removable: false,
    packTier: "core",
    itemKind: "item",
    preparationKind: "document",
    bag: "documents_folder",
    bulk: "small",
  }),
  templateItem(
    "general-doc-medical-card",
    "医保卡 / 社保卡",
    "documents",
    "must",
    "pack_now",
    {
      quantity: "实体卡 1 张 + 电子凭证 1 份",
      note: "用于就诊与医保结算；建议携带本人实体卡，并提前登录或截图保存电子凭证。",
      removable: false,
      packTier: "core",
      itemKind: "item",
      preparationKind: "document",
      bag: "documents_folder",
      bulk: "small",
    },
  ),
  templateItem(
    "general-doc-prenatal-records",
    "产检资料",
    "documents",
    "must",
    "pack_now",
    {
      quantity: "原件 1 套",
      note: "用于医护人员了解孕期检查记录；建议把产检本、影像和化验单按日期整理成一套。",
      removable: false,
      packTier: "core",
      itemKind: "item",
      preparationKind: "document",
      bag: "documents_folder",
      bulk: "medium",
    },
  ),
  templateItem("general-doc-admission", "入院资料", "documents", "must", "pack_now", {
    quantity: "医院已开具材料 1 套",
    note: "用于办理住院手续；建议把住院证、入院通知单等已开具材料与产检资料放在同一文件袋。",
    removable: false,
    packTier: "core",
    itemKind: "item",
    preparationKind: "document",
    bag: "documents_folder",
    bulk: "small",
  }),
  templateItem(
    "general-doc-birth-plan",
    "分娩偏好卡 / 出生计划",
    "documents",
    "recommended",
    "pack_now",
    {
      quantity: "打印 1-2 份",
      note: "用于快速说明个人偏好、过敏信息和联系人；建议控制在一页并打印一至两份，内容以现场沟通为准。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "document",
      bag: "documents_folder",
      bulk: "small",
    },
  ),
  templateItem(
    "general-doc-cash",
    "少量现金",
    "documents",
    "optional",
    "pack_now",
    {
      quantity: "约 200-500 元零钱",
      note: "用于手机支付不便或临时小额支出；建议只备少量零钱，具体金额按医院支付方式调整。",
      packTier: "optional",
      itemKind: "item",
      bag: "documents_folder",
      bulk: "small",
    },
  ),
  templateItem("general-doc-marriage-cert", "结婚证", "documents", "recommended", "pack_now", {
    quantity: "原件 1 本，复印件按当地要求",
    note: "部分地区办理相关材料时可能核验；建议先确认当地要求，再与户口本一起放入证件袋。",
    packTier: "core",
    itemKind: "item",
    preparationKind: "document",
    bag: "documents_folder",
    bulk: "small",
  }),
  templateItem("general-doc-hukou", "户口本", "documents", "recommended", "pack_now", {
    quantity: "原件 1 本，复印件按当地要求",
    note: "部分地区办理相关材料时可能核验；建议先确认当地要求，再与结婚证一起放入证件袋。",
    packTier: "core",
    itemKind: "item",
    preparationKind: "document",
    bag: "documents_folder",
    bulk: "small",
  }),
  templateItem(
    "general-doc-birth-registration",
    "生育服务单 / 生育登记凭证",
    "documents",
    "recommended",
    "pack_now",
    {
      quantity: "纸质或电子凭证 1 份",
      note: "用于当地生育登记信息核验；各地名称和形式不同，建议保存一份可离线出示的凭证。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "document",
      bag: "documents_folder",
      bulk: "small",
    },
  ),
  templateItem("general-doc-bank-card", "银行卡", "documents", "recommended", "pack_now", {
    quantity: "常用银行卡 1 张",
    note: "用于押金或手机支付受限时备用；建议携带一张常用卡，并提前确认额度和密码。",
    packTier: "core",
    itemKind: "item",
    preparationKind: "document",
    bag: "documents_folder",
    bulk: "small",
  }),

  templateItem("general-labor-phone", "手机", "mom_labor", "must", "grab_before_leaving", {
    quantity: "常用手机 1 部",
    note: "用于联系医院、家人和查看资料；建议出发前充足电，并把紧急联系人置顶。",
    removable: false,
    packTier: "core",
    itemKind: "item",
    preparationKind: "last_minute",
    bag: "last_minute",
    bulk: "small",
  }),
  templateItem(
    "general-labor-long-cable",
    "长充电线",
    "mom_labor",
    "must",
    "pack_now",
    {
      quantity: "长度约 2 米的线 1 条",
      note: "用于病床附近充电；建议准备约两米、接口匹配且外皮完好的充电线一条。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "pack_existing",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-labor-toiletries",
    "洗漱用品",
    "mom_labor",
    "must",
    "pack_now",
    {
      quantity: "旅行装 1 套",
      note: "用于住院期间日常清洁；建议用一个小包分装牙刷、牙膏、洁面和发圈等个人用品。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "pack_existing",
      bag: "mom_bag",
      bulk: "medium",
    },
  ),
  templateItem("general-labor-lip-balm", "护唇膏", "mom_labor", "must", "pack_now", {
    quantity: "无香型小支装 1 支",
    note: "用于空调环境或长时间说话时保持唇部舒适；建议带一支无香型小包装并单独收好。",
    packTier: "core",
    itemKind: "item",
    preparationKind: "pack_existing",
    bag: "mom_bag",
    bulk: "small",
  }),
  templateItem("general-labor-cup", "水杯 / 吸管杯", "mom_labor", "must", "pack_now", {
    quantity: "带盖杯 1 个，约 500-800 毫升",
    note: "用于住院期间补充饮水；建议选带盖、可单手开启且约五百至八百毫升的杯子。",
    packTier: "core",
    itemKind: "item",
    preparationKind: "pack_existing",
    bag: "mom_bag",
    bulk: "medium",
  }),
  templateItem("general-labor-eye-mask", "眼罩", "mom_labor", "optional", "pack_now", {
    quantity: "遮光眼罩 1 个",
    note: "用于病房灯光较亮时休息；建议选柔软、松紧可调且便于清洗的一只。",
    packTier: "optional",
    itemKind: "item",
    preparationKind: "pack_existing",
    bag: "mom_bag",
    bulk: "small",
  }),
  templateItem(
    "general-labor-pillow",
    "自用枕头，如需要",
    "mom_labor",
    "optional",
    "pack_now",
    {
      quantity: "枕头 1 个，按个人需要",
      note: "用于提高住院休息舒适度；如决定携带，建议套上易识别、可清洗的枕套。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "pack_existing",
      bag: "mom_bag",
      bulk: "large",
    },
  ),
  templateItem(
    "general-labor-cooling",
    "小风扇 / 喷雾瓶",
    "mom_labor",
    "optional",
    "pack_now",
    {
      quantity: "小风扇或喷雾瓶 1 个",
      note: "用于闷热环境下增加舒适感；建议选小型、低噪款，并先确认病区是否允许使用。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "pack_existing",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-labor-relaxation",
    "耳机 / 放松音频",
    "mom_labor",
    "optional",
    "grab_before_leaving",
    {
      quantity: "耳机 1 副，音频提前离线",
      note: "用于等待或休息时听熟悉的音频；建议准备一副已充电耳机，并提前下载离线内容。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "last_minute",
      bag: "last_minute",
      bulk: "small",
    },
  ),
  templateItem(
    "general-labor-tens",
    "TENS 镇痛仪，如已决定使用",
    "mom_labor",
    "optional",
    "pack_now",
    {
      quantity: "确认使用后带 1 套",
      note: "用于已与医护人员沟通确认的个人舒缓方案；建议只在允许使用时携带完整一套及匹配电源。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "pack_existing",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-labor-slippers",
    "防滑拖鞋 / 洗澡拖鞋",
    "mom_labor",
    "must",
    "pack_now",
    {
      quantity: "防滑、易穿脱拖鞋 1 双",
      note: "用于病房走动和洗漱；建议选鞋底防滑、脚面宽松且容易穿脱的一双。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "pack_existing",
      bag: "mom_bag",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-labor-glasses",
    "眼镜 / 隐形眼镜用品，如适用",
    "mom_labor",
    "must",
    "grab_before_leaving",
    {
      quantity: "框架眼镜 1 副 + 眼镜盒 1 个",
      note: "用于住院期间清晰视物；建议以框架眼镜为主，并把隐形眼镜护理用品按个人需要另装。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "last_minute",
      bag: "last_minute",
      bulk: "small",
    },
  ),
  templateItem(
    "general-labor-medicine-list",
    "常用药清单 / 医生确认用药",
    "mom_labor",
    "must",
    "grab_before_leaving",
    {
      note: "如有长期用药，请提前向医生或护士确认携带和使用方式。",
      packTier: "core",
      itemKind: "task",
      preparationKind: "last_minute",
      bag: "last_minute",
      bulk: "small",
    },
  ),
  templateItem(
    "general-labor-power-bank",
    "充电宝",
    "mom_labor",
    "optional",
    "pack_now",
    {
      quantity: "充电宝 1 个，约 10000-20000 毫安时",
      note: "用于插座不便时给手机补电；建议准备容量适中、状态正常且符合交通携带要求的一只。",
      packTier: "optional",
      itemKind: "item",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-labor-energy-food",
    "补能食品/饮料，按医院要求",
    "mom_labor",
    "optional",
    "pack_now",
    {
      quantity: "允许携带后备 1-2 份小包装",
      note: "用于等待时间较长时按需补充；建议先确认医院规定，再选便携、易开启的小包装。",
      packTier: "optional",
      itemKind: "item",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem("general-labor-towels", "毛巾", "mom_labor", "optional", "wash_before_pack", {
    quantity: "小毛巾 2 条",
    note: "用于擦脸、擦身时分开使用；建议准备两条颜色不同、易晾干的小毛巾。",
    packTier: "optional",
    itemKind: "item",
    bag: "mom_bag",
    bulk: "medium",
  }),
  templateItem(
    "general-labor-clothes",
    "住院换洗衣物 / 前开睡衣",
    "mom_labor",
    "recommended",
    "wash_before_pack",
    {
      quantity: "2-3 套",
      note: "用于住院期间换洗和护理操作；建议准备两至三套宽松、前开或易穿脱的衣物。",
      packTier: "optional",
      itemKind: "item",
      bag: "mom_bag",
      bulk: "large",
    },
  ),
  templateItem("general-labor-socks", "袜子", "mom_labor", "optional", "wash_before_pack", {
    quantity: "1-2 双",
    note: "用于空调环境下保暖和换洗；建议准备一至两双袜口宽松、透气的袜子。",
    packTier: "optional",
    itemKind: "item",
    bag: "mom_bag",
    bulk: "small",
  }),
  templateItem(
    "general-labor-tissues",
    "纸巾 / 湿巾",
    "mom_labor",
    "optional",
    "pack_now",
    {
      quantity: "各 1-2 包",
      note: "用于日常清洁和擦拭；建议纸巾、无香湿巾各带一至两小包，宝宝用品分开放。",
      packTier: "optional",
      itemKind: "item",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-labor-ctg-belt",
    "胎心监护带",
    "mom_labor",
    "recommended",
    "pack_now",
    {
      quantity: "2 条",
      note: "用于医院明确要求自带时配合胎心监护；建议产检时先确认规格，需要时准备两条。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-labor-straws",
    "一次性吸管",
    "mom_labor",
    "recommended",
    "pack_now",
    {
      quantity: "独立包装 5-10 根",
      note: "用于不便起身时饮水；建议带五至十根独立包装、与杯口匹配的吸管。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-labor-tableware",
    "餐具 / 饭盒筷勺",
    "mom_labor",
    "optional",
    "pack_now",
    {
      quantity: "1 套",
      note: "用于医院不提供餐具或家属送餐时使用；建议准备一套带盖、易清洗的饭盒和筷勺。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "wash_then_pack",
      bag: "mom_bag",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-labor-moon-toothbrush",
    "软毛牙刷 / 漱口水",
    "mom_labor",
    "recommended",
    "pack_now",
    {
      quantity: "软毛牙刷 1 支 + 小瓶漱口水 1 瓶",
      note: "用于住院期间保持口腔清洁；普通软毛牙刷即可，不必专门购买“月子牙刷”，漱口水按个人习惯带小瓶装。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "small",
    },
  ),

  templateItem(
    "general-postpartum-pads",
    "产褥垫 / 产后卫生巾",
    "mom_postpartum",
    "must",
    "pack_now",
    {
      quantity: "10-20 片",
      note: "用于住院期间铺垫床面和按需更换；建议先备六十乘九十厘米产褥垫十至二十片，并向医院确认是否还需产后卫生巾。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "medium",
    },
  ),
  templateItem("general-postpartum-paper", "刀纸 / 产褥卫生纸", "mom_postpartum", "must", "pack_now", {
    quantity: "1-2 提",
    note: "用于医院明确要求的产房或病房护理场景；建议先确认规格，再准备一至两提，医院提供则无需重复携带。",
    packTier: "core",
    itemKind: "item",
    preparationKind: "buy_and_pack",
    bag: "mom_bag",
    bulk: "medium",
  }),
  templateItem(
    "general-postpartum-metered-pads",
    "计量型卫生巾",
    "mom_postpartum",
    "recommended",
    "pack_now",
    {
      quantity: "医院要求时 1 小包",
      note: "用于医院有明确计量记录要求的场景；建议只在医护人员要求时购买一小包，并确认具体规格。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-postpartum-underwear",
    "一次性内裤",
    "mom_postpartum",
    "must",
    "pack_now",
    {
      quantity: "预计住院天数 + 2 条，通常 5-7 条",
      note: "用于住院期间便捷换洗；建议按预计住院天数多备两条，并按孕晚期实际腰臀围选码，不照搬孕前尺码。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-postpartum-toilet-seat-covers",
    "一次性马桶垫",
    "mom_postpartum",
    "recommended",
    "pack_now",
    {
      quantity: "独立包装 10-20 片",
      note: "用于使用病房公共马桶时按个人习惯铺垫；建议带十至二十片独立包装、大小合适的款式。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-postpartum-yuezi-hat-shoes",
    "月子帽 / 月子鞋",
    "mom_postpartum",
    "optional",
    "wash_before_pack",
    {
      quantity: "帽子或软底鞋各 1 件，按需",
      note: "用于出院或病房内按个人习惯保暖；建议根据季节选轻便、透气的单件用品，不必成套购买。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "wash_then_pack",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-postpartum-going-home-clothes",
    "宽松出院衣物",
    "mom_postpartum",
    "must",
    "wash_before_pack",
    {
      quantity: "1 套",
      note: "用于出院当天舒适穿着；建议准备一套适合季节、腹部宽松的衣物，并按孕晚期尺码试穿。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "wash_then_pack",
      bag: "mom_bag",
      bulk: "large",
    },
  ),
  templateItem(
    "general-postpartum-storage-bags",
    "收纳袋 / 脏衣袋",
    "mom_postpartum",
    "must",
    "pack_now",
    {
      quantity: "3-5 个",
      note: "用于分开放置干净用品、湿物和脏衣物；建议准备三至五个不同尺寸、可封口的收纳袋。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "pack_existing",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-postpartum-nursing-bra",
    "哺乳内衣",
    "mom_postpartum",
    "recommended",
    "wash_before_pack",
    {
      quantity: "2-3 件",
      note: "用于计划哺乳时方便穿脱和换洗；建议准备两至三件可调节、无明显勒压感的款式，并按孕晚期实际胸围留余量。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "wash_then_pack",
      bag: "mom_bag",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-postpartum-breast-pads",
    "溢乳垫",
    "mom_postpartum",
    "optional",
    "pack_now",
    {
      quantity: "先备 1 小包，约 10-20 片",
      note: "用于计划哺乳时按需保持衣物干爽；建议先备十至二十片小包装，使用量稳定后再补充。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-postpartum-nipple-cream",
    "乳头膏",
    "mom_postpartum",
    "optional",
    "pack_now",
    {
      quantity: "小支装 1 支，按需",
      note: "用于计划哺乳时按个人需要进行皮肤护理；建议选成分简单的小支装，使用方式以产品说明和专业建议为准。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-postpartum-pull-up-pants",
    "产妇拉拉裤 / 安心裤",
    "mom_postpartum",
    "recommended",
    "pack_now",
    {
      quantity: "1 包（5-8 片）",
      note: "用于住院初期方便穿脱和更换；建议先备五至八片小包装，并按孕晚期实际腰臀围选择尺码。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-postpartum-adult-diapers",
    "成人纸尿裤",
    "mom_postpartum",
    "optional",
    "pack_now",
    {
      quantity: "按需 1 小包",
      note: "用于部分医院产房或产后明确要求时；建议先向医院确认是否需要，不要求则不必携带。常见品牌：包大人。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-postpartum-peri-bottle",
    "私处冲洗器",
    "mom_postpartum",
    "recommended",
    "pack_now",
    {
      quantity: "1 个",
      note: "用于医院建议进行局部清洁时辅助冲洗；建议准备一个约三百至五百毫升、弯嘴且便于清洗的瓶子。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-postpartum-cold-pack",
    "会阴冷敷贴",
    "mom_postpartum",
    "optional",
    "pack_now",
    {
      quantity: "3-5 片",
      note: "顺产后如医护建议，可用于短时冷敷、缓解会阴肿胀不适；建议先备三至五片，使用时避免直接接触皮肤，并确认医院是否提供或允许使用。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-postpartum-basins",
    "脸盆 / 折叠盆",
    "mom_postpartum",
    "optional",
    "pack_now",
    {
      quantity: "2 个，妈妈和宝宝分开",
      note: "用于妈妈局部清洁和宝宝洗脸洗屁屁；建议准备两个颜色可区分、易清洗的盆，折叠款更省空间。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-postpartum-belly-wrap",
    "收腹带",
    "mom_postpartum",
    "optional",
    "pack_now",
    {
      quantity: "1 条",
      note: "用于医护人员建议使用时提供适度支撑；建议先确认是否需要，再按孕晚期围度选择一条可调节款。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-postpartum-breast-pump",
    "吸奶器",
    "mom_postpartum",
    "recommended",
    "pack_now",
    {
      quantity: "1 套",
      note: "用于计划哺乳且确有挤奶需要时备用；建议带一套已试装设备，电动款充好电，接触奶液的配件按说明清洁后装袋。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "wash_then_pack",
      bag: "mom_bag",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-postpartum-milk-bags",
    "储奶袋",
    "mom_postpartum",
    "optional",
    "pack_now",
    {
      quantity: "先备 10-20 片",
      note: "用于确有储存母乳需要时分装和标记；建议先备十至二十片小容量袋，储存方式按医院指导执行。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "mom_bag",
      bulk: "small",
    },
  ),

  templateItem("general-baby-home-clothes", "宝宝出院衣物", "baby", "must", "wash_before_pack", {
    quantity: "1 套 + 备用 1 套",
    note: "用于宝宝出院当天穿着和临时替换；建议准备一主一备两套柔软连体衣，尺码按预估体重在五十二或五十九码中选择。",
    packTier: "core",
    itemKind: "item",
    preparationKind: "wash_then_pack",
    bag: "baby_bag",
    bulk: "medium",
  }),
  templateItem("general-baby-blanket", "包被 / 小毯子", "baby", "must", "wash_before_pack", {
    quantity: "1 条，寒冷季节可备 2 条",
    note: "用于出院路上包裹和按需保暖；建议按季节准备一条透气、易清洗的款式，寒冷天气可加一条备用。",
    packTier: "core",
    itemKind: "item",
    preparationKind: "wash_then_pack",
    bag: "baby_bag",
    bulk: "large",
  }),
  templateItem("general-baby-diapers", "尿不湿", "baby", "must", "pack_now", {
    quantity: "NB 码先备 1 小包，约 24-36 片",
    note: "用于住院期间按需更换；建议 NB 码先买一小包，出生后再按宝宝体重、腰围和贴合情况决定是否换码。",
    packTier: "core",
    itemKind: "item",
    preparationKind: "buy_and_pack",
    bag: "baby_bag",
    bulk: "medium",
  }),
  templateItem("general-baby-wipes", "湿巾 / 棉柔巾", "baby", "must", "pack_now", {
    quantity: "无香型湿巾或棉柔巾 1-2 小包",
    note: "用于换洗时擦拭皮肤或清理台面；建议准备一至两小包无香型产品，并先确认医院是否提供。",
    packTier: "core",
    itemKind: "item",
    preparationKind: "buy_and_pack",
    bag: "baby_bag",
    bulk: "medium",
  }),
  templateItem("general-baby-hat", "宝宝帽子", "baby", "optional", "wash_before_pack", {
    quantity: "柔软帽子 1 顶，按季节",
    note: "用于出院路上按天气挡风保暖；建议准备一顶无硬饰、松紧合适且与季节匹配的帽子。",
    packTier: "optional",
    itemKind: "item",
    preparationKind: "wash_then_pack",
    bag: "baby_bag",
    bulk: "small",
  }),
  templateItem("general-baby-socks", "宝宝袜子", "baby", "optional", "wash_before_pack", {
    quantity: "袜口宽松的袜子 1-2 双",
    note: "用于出院途中按季节保暖；建议准备一至两双袜口宽松、无线头明显残留的袜子。",
    packTier: "optional",
    itemKind: "item",
    preparationKind: "wash_then_pack",
    bag: "baby_bag",
    bulk: "small",
  }),
  templateItem(
    "general-baby-hospital-clothes",
    "宝宝住院衣物（连体衣/和尚服），如医院不提供",
    "baby",
    "optional",
    "wash_before_pack",
    {
      quantity: "2-3 套",
      note: "用于医院要求自带时给宝宝住院换洗；建议先确认是否提供，再准备两至三套柔软、易穿脱的衣物。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "wash_then_pack",
      bag: "baby_bag",
      bulk: "medium",
    },
  ),
  templateItem("general-baby-towels", "纱布巾 / 小方巾", "baby", "optional", "wash_before_pack", {
    quantity: "3-5 条",
    note: "用于喂奶垫巾、拍嗝和日常擦拭；建议准备三至五条颜色或图案易区分、洗后易干的小方巾。",
    packTier: "optional",
    itemKind: "item",
    preparationKind: "wash_then_pack",
    bag: "baby_bag",
    bulk: "small",
  }),
  templateItem("general-baby-diaper-cream", "护臀膏", "baby", "optional", "pack_now", {
    quantity: "小支装 1 支，按需",
    note: "用于换洗后按个人护理方案隔离潮湿；建议只带一支成分简单的小包装，使用按说明或专业建议进行。",
    packTier: "optional",
    itemKind: "item",
    bag: "baby_bag",
    bulk: "small",
  }),
  templateItem("general-baby-changing-pads", "一次性隔尿垫", "baby", "optional", "pack_now", {
    quantity: "10-20 片",
    note: "用于换尿布时保护床面或护理台；建议准备十至二十片约四十五乘六十厘米的小包装。",
    packTier: "optional",
    itemKind: "item",
    preparationKind: "buy_and_pack",
    bag: "baby_bag",
    bulk: "medium",
  }),
  templateItem(
    "general-baby-formula-bottle",
    "小罐奶粉 + 奶瓶 / 硅胶小勺（备用）",
    "baby",
    "optional",
    "confirm_beforehand",
    {
      quantity: "确认需要后，小包装 1 份 + 喂养工具 1 套",
      note: "用于医院明确允许且家庭决定准备备用喂养用品的场景；建议先确认医院政策，再准备最小包装和一套易清洗工具。",
      packTier: "confirm",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "baby_bag",
      bulk: "medium",
    },
  ),
  templateItem("general-baby-nail-clipper", "宝宝指甲剪", "baby", "optional", "pack_now", {
    quantity: "1 个",
    note: "用于确有修剪需要时整理指甲边缘；建议准备一个圆头、握持稳定且带保护盖的小剪具。",
    packTier: "hidden",
    itemKind: "item",
    bag: "baby_bag",
    bulk: "small",
  }),
  templateItem(
    "general-baby-navel-care",
    "护脐贴 + 碘伏棉签",
    "baby",
    "recommended",
    "pack_now",
    {
      quantity: "医院明确要求时各 1 小份",
      note: "用于医院明确要求的脐部护理用品准备；建议先询问是否需要及具体规格，不自行增加处理步骤。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "baby_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-baby-cotton-swabs",
    "婴儿细轴棉签",
    "baby",
    "optional",
    "pack_now",
    {
      quantity: "1 盒",
      note: "用于皮肤表面或耳廓外侧的轻柔清洁；建议选细轴纸杆小包装，并避免伸入鼻腔或耳道。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "baby_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-baby-lotion",
    "婴儿润肤霜 / 抚触油",
    "baby",
    "optional",
    "pack_now",
    {
      quantity: "无香型 30-50 毫升小瓶 1 个",
      note: "用于干燥季节按需进行皮肤保湿；建议先备三十至五十毫升无香型小瓶，使用前按说明少量尝试。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "baby_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-baby-bottle-brush",
    "奶瓶刷 + 小瓶清洗剂",
    "baby",
    "optional",
    "pack_now",
    {
      quantity: "确认使用奶瓶后，各 1 个小规格",
      note: "用于确实使用奶瓶时清洁接触奶液的配件；建议准备一把尺寸匹配的刷具和一小瓶清洗剂，用后按说明清洗晾干。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "baby_bag",
      bulk: "small",
    },
  ),
  templateItem(
    "general-baby-laundry",
    "小衣架 + 婴儿洗衣皂",
    "baby",
    "optional",
    "pack_now",
    {
      quantity: "小衣架 2-3 个 + 洗衣皂 1 块",
      note: "用于住院期间手洗宝宝小件衣物并晾起来；建议准备两三个小衣架和一小块婴儿洗衣皂。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_and_pack",
      bag: "baby_bag",
      bulk: "small",
    },
  ),

  templateItem(
    "general-confinement-mom-yuezi-clothes",
    "月子服",
    "confinement_mom",
    "recommended",
    "prepare_now",
    {
      quantity: "纯棉哺乳款 2-3 套",
      note: "用于月子期间换洗；产后出汗多、易溢奶，建议选纯棉透气、前开扣或带哺乳口的款式，到手先清洗。常见品牌：嫚熙、babycare。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-confinement-mom-pads-stock",
    "夜用卫生巾 / 安心裤（家中囤货）",
    "confinement_mom",
    "recommended",
    "prepare_now",
    {
      quantity: "420 毫米夜用 2-3 包或安心裤 1-2 包",
      note: "用于月子期间持续约四至六周的恶露；建议前期用加长夜用或安心裤，量变少后换普通款。常见品牌：高洁丝、苏菲、她研社。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-confinement-mom-underwear-stock",
    "一次性内裤（家中补充）",
    "confinement_mom",
    "recommended",
    "prepare_now",
    {
      quantity: "按需补充 10-20 条",
      note: "用于恶露期换洗频繁时直接更换；住院带的数量不够时提前补足。常见品牌：嫚熙、十月结晶。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-mom-nursing-pads-stock",
    "防溢乳垫（家中补充）",
    "confinement_mom",
    "optional",
    "prepare_now",
    {
      quantity: "先备 1 小盒，用量稳定后再补",
      note: "用于母乳量稳定前防止溢奶弄脏衣物；每个人的溢奶量差异很大，先接着住院小包少量试用，确认材质合适和实际用量后再补，不必提前囤整箱。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-mom-milk-bags-stock",
    "储奶袋（家中补充）",
    "confinement_mom",
    "optional",
    "prepare_now",
    {
      quantity: "先备 10-20 片，确认有存奶需求后再补",
      note: "用于确有存奶需求时分装冷冻；先用十至二十片小容量袋，确认泌乳、喂养和存奶节奏后再补，避免在尚未使用前大量囤货。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-mom-pumping-bra",
    "免手扶吸奶文胸",
    "confinement_mom",
    "optional",
    "prepare_now",
    {
      quantity: "确认使用双边吸奶器后备 1 件",
      note: "用于双边吸奶时固定喇叭罩、腾出双手；先确认吸奶器类型和实际使用频率，可调节或能利用现有哺乳内衣的方案更省，确定需要后再买。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-mom-nursing-pillow",
    "哺乳枕",
    "confinement_mom",
    "optional",
    "prepare_now",
    {
      quantity: "1 个",
      note: "用于喂奶时垫高手臂、减轻腰背负担；非必备，也可先用孕妇枕或靠垫代替。常见品牌：嫚熙、佳韵宝。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "large",
    },
  ),
  templateItem(
    "general-confinement-mom-diaper-bag",
    "妈咪包",
    "confinement_mom",
    "optional",
    "prepare_now",
    {
      quantity: "1 个",
      note: "用于带宝宝体检、打疫苗时分区收纳出门用品；不着急，出生后按需要再买。常见品牌：嫚熙、babycare。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "medium",
    },
  ),

  templateItem(
    "general-confinement-baby-diapers-stock",
    "纸尿裤囤货（NB + S 码）",
    "confinement_baby",
    "must",
    "prepare_now",
    {
      quantity: "NB 试用装或小包 1-2 包，S 码暂不多囤",
      note: "用于月子里高频更换；不同品牌版型、宝宝体重和皮肤适应情况差异很大，先备试用装或一两小包，出生后确认不漏尿、不勒腿再按码数补货。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "large",
    },
  ),
  templateItem(
    "general-confinement-baby-bottles",
    "奶瓶",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "160 毫升 + 240 毫升各 1 个，配 SS/S 码奶嘴",
      note: "用于母乳不足或瓶喂时；建议先各备一个，奶嘴一至两个月更换一次。常见品牌：贝亲、hegen、世喜、布朗博士。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-formula",
    "一段奶粉",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "一段小罐（约 400 克）1 罐",
      note: "用于家庭确定需要配方奶时备用；只备一罐小包装，不提前囤多罐，喂养方案优先结合宝宝情况和专业建议，未使用时保持密封并留意有效期。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-confinement-baby-kettle",
    "恒温水壶 / 调奶器",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "1 个",
      note: "用于奶粉喂养家庭随时冲奶；建议设在 45℃ 左右随取随冲，母乳亲喂为主可先不买。常见品牌：波咯咯、小白熊、可优比。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-confinement-baby-sterilizer",
    "奶瓶消毒器 / 消毒柜",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "1 台",
      note: "用于奶瓶奶嘴和餐具日常消毒；建议选紫外线灯珠款、约 20 升容量，后期玩具餐具也能用。常见品牌：波咯咯、大宇、小白熊。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "large",
    },
  ),
  templateItem(
    "general-confinement-baby-bottle-warmer",
    "温奶器",
    "confinement_baby",
    "optional",
    "prepare_now",
    {
      quantity: "1 个",
      note: "用于瓶喂母乳时均匀温奶；使用频率不确定，需要时再买也来得及。常见品牌：小白熊、波咯咯。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-pacifier",
    "安抚奶嘴",
    "confinement_baby",
    "optional",
    "prepare_now",
    {
      quantity: "1-2 个",
      note: "用于宝宝需要安抚时备用；非必备，注意按月龄选型号并勤清洗。常见品牌：新安怡、世喜。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-wash",
    "洗发沐浴二合一",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "按压式 1 瓶",
      note: "用于给宝宝洗澡；新生儿每周用一至两次即可，建议选温和无泪配方的按压款。常见品牌：艾惟诺、施巴。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-face-cream",
    "婴儿面霜",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "1 瓶",
      note: "用于干燥季节面部保湿；建议先小面积试用，湿疹宝宝更要坚持厚涂保湿。常见品牌：妙思乐、松达、顺峰宝宝。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-body-lotion",
    "婴儿身体乳",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "1 瓶",
      note: "用于洗澡后全身薄涂保湿；大罐装耐用，全家也能一起用。常见品牌：丝塔芙大白罐、艾惟诺、松达。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-peach-water",
    "桃子水（液体爽身粉）",
    "confinement_baby",
    "optional",
    "prepare_now",
    {
      quantity: "1 瓶",
      note: "用于夏季预防痱子和淹脖子；建议选液体款不用粉状，其他季节可不买。常见品牌：宇津、chuchu、贝亲。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-bath-towels",
    "纱布浴巾",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "六层纱布 2-3 条",
      note: "用于洗澡后包裹擦干；夏天还能当包被或小盖毯，建议多备一条换洗。常见品牌：全棉时代、嫚熙。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-confinement-baby-bibs",
    "口水巾",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "4-6 条",
      note: "用于拍嗝垫肩、接吐奶和擦口水；消耗量大，建议多备几轮换。常见品牌：全棉时代、嫚熙。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-bathtub",
    "折叠洗澡盆 + 浴网",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "1 套",
      note: "用于在家给宝宝洗澡；建议选可折叠款好收纳，浴网比浴垫更不易发霉。常见品牌：蒂爱、嫚熙。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "large",
    },
  ),
  templateItem(
    "general-confinement-baby-bath-thermometer",
    "水温计",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "1 个",
      note: "用于洗澡前测水温；建议控制在 36-37℃、不超过 40℃，冬天或感冒时可稍高。常见品牌：嫚熙。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-soft-tissues",
    "云柔巾 + 棉柔巾囤货",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "各 1 提，按消耗补",
      note: "用于擦脸擦口水和打湿洗屁屁；消耗量大可以多囤，棉柔巾建议选全棉材质。常见品牌：可心柔、全棉时代、德佑。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-confinement-baby-changing-pads-stock",
    "一次性隔尿垫囤货",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "2 包，约 160 片",
      note: "用于换尿布、晾屁屁时垫台面；妈妈用剩的产褥垫也能替代，可按消耗慢慢补。常见品牌：德佑。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-confinement-baby-washable-pads",
    "可水洗隔尿垫",
    "confinement_baby",
    "optional",
    "prepare_now",
    {
      quantity: "2 条，买大不买小",
      note: "用于宝宝大一点后垫床、垫尿布台反复使用；和一次性款搭配更省。常见品牌：littletiny、嫚熙、诺绵。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-confinement-baby-laundry-detergent",
    "婴儿洗衣液",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "1-2 瓶",
      note: "用于宝宝衣物和口水巾天天清洗；建议选无荧光剂、温和配方。常见品牌：保宁、英氏、babycare。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-confinement-baby-hangers",
    "宝宝小衣架",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "20-30 个",
      note: "用于晾晒宝宝小件衣物；可伸缩款能挂成一串，节省空间。常见品牌：蒂爱。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-bodysuit",
    "半背衣 / 包屁衣",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "52 码 2 件",
      note: "用于月子期间日常穿着、方便换尿布；建议选纯棉 A 类，到手先清洗。常见品牌：乖奇熊、全棉时代、aqpa。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-confinement-baby-rompers",
    "宝宝连体衣囤货",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "52、59 码各 2 套",
      note: "用于宝宝出生后换洗；宝宝长得快，建议先备两个码，出生后再追加。常见品牌：乖奇熊、全棉时代、aqpa。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-confinement-baby-sleep-sack",
    "防惊跳睡袋",
    "confinement_baby",
    "optional",
    "prepare_now",
    {
      quantity: "1-2 件",
      note: "用于小月龄睡觉时减少惊跳反射；投降式更贴合，也可先用包被替代。常见品牌：lovetodream、嫚熙、贝肽斯。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-confinement-baby-belly-band",
    "宝宝肚围",
    "confinement_baby",
    "optional",
    "prepare_now",
    {
      quantity: "2 件",
      note: "用于换尿布、洗澡时护住肚子不受凉；选纯棉弹性款即可。常见品牌：嫚熙、童泰。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-thermometer",
    "耳温枪 / 体温计",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "1 个",
      note: "用于宝宝发热时快速测温；耳温枪几秒出数，也可先备电子腋温计。常见品牌：博朗。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-nasal-care",
    "婴儿生理盐水 + 手动吸鼻器",
    "confinement_baby",
    "optional",
    "prepare_now",
    {
      quantity: "无药物生理盐水 1 小瓶 + 手动吸鼻器 1 个",
      note: "用于鼻塞时先滴入不含药物的生理盐水、再按需轻柔吸出分泌物；不把吸头深入鼻腔，也不自行加入减充血药，持续呼吸困难应及时就医。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-medicine-syringe",
    "毫升刻度口服喂药器",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "1 支，按需使用",
      note: "用于需要口服药物时按毫升准确量取；只按药品标签或医护人员给出的剂量使用，不用家用汤匙估量，每次使用后按产品说明清洁晾干。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-vitamin-ad",
    "维生素 AD / D3",
    "confinement_baby",
    "must",
    "prepare_now",
    {
      quantity: "1 盒",
      note: "用于出生约 15 天后每天补充一滴，直接滴入嘴里；具体选 AD 还是 D3 遵医嘱。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-zinc-oxide",
    "氧化锌软膏",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "1 罐",
      note: "用于红屁屁时加强隔离防护；破皮或持续不见好请就医，日常护理仍用护臀膏。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-calamine",
    "炉甘石洗剂",
    "confinement_baby",
    "optional",
    "prepare_now",
    {
      quantity: "1 瓶",
      note: "用于痱子、湿疹止痒；皮肤破溃处不要用，用药前先咨询医生。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-black-white-cards",
    "黑白卡",
    "confinement_baby",
    "optional",
    "prepare_now",
    {
      quantity: "1 套",
      note: "用于出生两周后练追视；每次几十秒即可，可搭配小沙锤练听力和抓握。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-night-light",
    "小夜灯",
    "confinement_baby",
    "optional",
    "prepare_now",
    {
      quantity: "1 个，暖光可调",
      note: "用于半夜喂奶、换尿布时照明；建议选遥控或拍拍款，不用开大灯。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-hair-clipper",
    "婴儿理发器",
    "confinement_baby",
    "optional",
    "prepare_now",
    {
      quantity: "1 套",
      note: "用于满月后在家理发；不着急买，需要时再选静音款。常见品牌：樱舒、可优比。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-confinement-baby-crib",
    "婴儿床 + 床上用品",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "1 套",
      note: "用于宝宝在家独立安睡；按安全睡眠要求选择结构牢固、床垫贴合的产品，二手可考虑，但要确认无缺件、无召回、无损坏，并配齐原说明书。",
      packTier: "core",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "large",
    },
  ),
  templateItem(
    "general-confinement-baby-changing-table",
    "尿布台",
    "confinement_baby",
    "optional",
    "prepare_now",
    {
      quantity: "1 个",
      note: "用于站着换尿布、做抚触，减轻弯腰；也可用床上隔尿垫代替。常见品牌：honeyseed、可优比。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "large",
    },
  ),
  templateItem(
    "general-confinement-baby-stroller",
    "婴儿推车",
    "confinement_baby",
    "recommended",
    "prepare_now",
    {
      quantity: "1 辆",
      note: "用于出月子后遛娃；月子里基本用不到，可以出生后试推再买。二手可考虑，但要检查车架、刹车、折叠锁和安全带是否完整可靠。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "large",
    },
  ),
  templateItem(
    "general-confinement-baby-washer",
    "婴儿洗衣机",
    "confinement_baby",
    "optional",
    "prepare_now",
    {
      quantity: "1 台",
      note: "用于宝宝衣物单独清洗；带煮洗功能更好，预算有限也可手洗。常见品牌：海信、美的。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "large",
    },
  ),
  templateItem(
    "general-confinement-baby-storage-cart",
    "置物小推车",
    "confinement_baby",
    "optional",
    "prepare_now",
    {
      quantity: "1 辆",
      note: "用于把尿布、湿巾和护肤品集中收纳，推到哪用到哪。常见品牌：也雅。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "buy_for_home",
      bag: "none",
      bulk: "medium",
    },
  ),

  templateItem(
    "general-partner-doc-folder",
    "确认证件包位置",
    "partner",
    "must",
    "pack_now",
    {
      note: "证件包放在固定位置，全家都知道在哪儿。",
      packTier: "core",
      itemKind: "task",
      preparationKind: "task",
      bag: "documents_folder",
      bulk: "small",
    },
  ),
  templateItem(
    "general-partner-payment",
    "确认支付方式和住院押金",
    "partner",
    "must",
    "confirm_beforehand",
    {
      note: "支付方式、住院押金和医保结算属于入院确认事项，不算待产包物品。若医院明确要求实体银行卡或现金，再作为自定义物品加入证件包。",
      packTier: "core",
      itemKind: "task",
      preparationKind: "task",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-partner-car-seat",
    "确认安全座椅安装",
    "partner",
    "must",
    "pack_now",
    {
      note: "用于提前熟悉宝宝返家乘车安排；选择明确适用于新生儿身高和体重的座椅，按说明书和车辆条件完成后向安装并练习卡扣。二手座椅只考虑来源可靠、无碰撞史、未召回、未过期且配件说明书齐全的产品。",
      packTier: "core",
      itemKind: "task",
      preparationKind: "install_or_place",
      bag: "car",
      bulk: "large",
    },
  ),
  templateItem(
    "general-partner-family-notice",
    "临出门通知家人/紧急联系人",
    "partner",
    "recommended",
    "grab_before_leaving",
    {
      note: "出发时发一条消息：去哪家医院、走哪个门、谁在场。",
      packTier: "core",
      itemKind: "task",
      preparationKind: "task",
      bag: "last_minute",
      bulk: "small",
    },
  ),
  templateItem(
    "general-partner-id",
    "陪产人身份证件",
    "partner",
    "must",
    "grab_before_leaving",
    {
      quantity: "原件 1 份",
      note: "用于陪产登记或身份核验；建议把原件一份放入陪产人随身包，并先确认医院要求。",
      packTier: "core",
      itemKind: "item",
      bag: "dad_backpack",
      bulk: "small",
    },
  ),
  templateItem(
    "general-partner-charger",
    "手机充电器",
    "partner",
    "must",
    "pack_now",
    {
      quantity: "1 套",
      note: "用于陪产期间给常用设备充电；建议准备一套接口匹配的充电器和约两米长线。",
      packTier: "core",
      itemKind: "item",
      bag: "dad_backpack",
      bulk: "small",
    },
  ),
  templateItem("general-partner-water-snacks", "水和零食", "partner", "recommended", "pack_now", {
    quantity: "饮水 1-2 瓶 + 小包装零食 1-2 份",
    note: "用于陪产人等待或无法及时用餐时补充；建议准备一至两瓶饮水和一至两份易开启的小包装零食。",
    packTier: "core",
    itemKind: "item",
    bag: "dad_backpack",
    bulk: "medium",
  }),
  templateItem(
    "general-partner-clothes",
    "换洗衣物 1 套，如需过夜",
    "partner",
    "recommended",
    "wash_before_pack",
    {
      quantity: "换洗衣物 1 套",
      note: "用于陪产人需要过夜时换洗；建议准备一套轻便、易穿脱且适合病房温度的衣物。",
      packTier: "core",
      itemKind: "item",
      bag: "dad_backpack",
      bulk: "medium",
    },
  ),
  templateItem(
    "general-partner-toiletries",
    "洗漱用品，如需过夜",
    "partner",
    "recommended",
    "pack_now",
    {
      quantity: "旅行装 1 套",
      note: "用于陪产人过夜时日常清洁；建议用一个小包分装牙刷、牙膏和小毛巾各一件。",
      packTier: "core",
      itemKind: "item",
      bag: "dad_backpack",
      bulk: "small",
    },
  ),
  templateItem(
    "general-partner-glasses",
    "眼镜 / 隐形眼镜用品",
    "partner",
    "optional",
    "grab_before_leaving",
    {
      quantity: "框架眼镜 1 副 + 眼镜盒 1 个",
      note: "用于陪产期间清晰视物和轮换佩戴；建议带一副框架眼镜及保护盒，其他护理用品按个人需要准备。",
      packTier: "optional",
      itemKind: "item",
      bag: "dad_backpack",
      bulk: "small",
    },
  ),
  templateItem(
    "general-partner-medicine-list",
    "个人常用药清单 / 按需确认",
    "partner",
    "optional",
    "grab_before_leaving",
    {
      note: "如有长期用药，按个人情况提前确认。",
      packTier: "optional",
      itemKind: "task",
      bag: "dad_backpack",
      bulk: "small",
    },
  ),
  templateItem(
    "general-partner-bedding",
    "陪护被褥 / 折叠床",
    "partner",
    "optional",
    "pack_now",
    {
      quantity: "1 套",
      note: "用于医院允许且确需过夜陪护时休息；建议先确认病区提供情况和尺寸限制，再决定是否带一套轻便用品。",
      packTier: "optional",
      itemKind: "item",
      preparationKind: "pack_existing",
      bag: "car",
      bulk: "large",
    },
  ),

  templateItem(
    "general-going-home-mom-clothes",
    "妈妈出院衣物",
    "going_home",
    "must",
    "wash_before_pack",
    {
      quantity: "宽松衣物 1 套",
      note: "用于妈妈出院当天舒适穿着；建议准备一套适合天气、腹部宽松并按孕晚期尺码试穿的衣物。",
      packTier: "core",
      itemKind: "item",
      bag: "mom_bag",
      bulk: "large",
    },
  ),
  templateItem(
    "general-going-home-baby-clothes",
    "宝宝出院衣物",
    "going_home",
    "must",
    "wash_before_pack",
    {
      quantity: "当日 1 套 + 备用 1 套",
      note: "用于宝宝出院当天穿着和临时替换；建议一主一备两套，并按预估体重和天气选择尺码厚度。",
      packTier: "core",
      itemKind: "item",
      bag: "baby_bag",
      bulk: "medium",
    },
  ),
  templateItem("general-going-home-blanket", "包被 / 小毯子", "going_home", "must", "wash_before_pack", {
    quantity: "1 条",
    note: "用于宝宝出院途中按需包裹；建议准备一条与天气匹配、透气且不影响安全座椅约束带贴合的款式。",
    packTier: "core",
    itemKind: "item",
    bag: "baby_bag",
    bulk: "large",
  }),
  templateItem(
    "general-going-home-transport",
    "返家交通安排",
    "going_home",
    "must",
    "confirm_beforehand",
    {
      note: "提前约好车，确认能装安全座椅，堵车时段多留余量。",
      packTier: "core",
      itemKind: "task",
      preparationKind: "install_or_place",
      bag: "car",
      bulk: "small",
    },
  ),
  templateItem(
    "general-going-home-car-seat",
    "安全座椅确认",
    "going_home",
    "must",
    "pack_now",
    {
      note: "用于宝宝出院返程时提供合适约束；确认座椅适用于新生儿身高和体重，按说明书后向安装，并在出院前复核固定状态和卡扣路径。",
      packTier: "core",
      itemKind: "task",
      preparationKind: "install_or_place",
      bag: "car",
      bulk: "large",
    },
  ),

  templateItem("general-last-id", "身份证件", "last_minute", "must", "grab_before_leaving", {
    note: "出门前把需要携带的身份证原件放入证件包，并由陪产人按医院要求复核一次。",
    removable: false,
    packTier: "core",
    itemKind: "task",
    preparationKind: "last_minute",
    bag: "last_minute",
    bulk: "small",
  }),
  templateItem(
    "general-last-medical-card",
    "医保卡 / 社保卡",
    "last_minute",
    "must",
    "grab_before_leaving",
    {
      note: "出门前把本人医保卡或社保卡放入证件包，并确认电子凭证能够正常打开。",
      removable: false,
      packTier: "core",
      itemKind: "task",
      preparationKind: "last_minute",
      bag: "last_minute",
      bulk: "small",
    },
  ),
  templateItem(
    "general-last-maternal-book",
    "本地区孕产资料 / 母子健康手册",
    "last_minute",
    "must",
    "grab_before_leaving",
    {
      note: "出门前把本地区要求的孕产资料或母子健康手册放入证件包，并确认纸质或电子版本可出示。",
      removable: false,
      packTier: "core",
      itemKind: "task",
      preparationKind: "last_minute",
      bag: "last_minute",
      bulk: "small",
    },
  ),
  templateItem(
    "general-last-prenatal-records",
    "产检资料",
    "last_minute",
    "must",
    "grab_before_leaving",
    {
      note: "出门前把按日期整理好的产检本、影像和化验资料整套放入证件包。",
      removable: false,
      packTier: "core",
      itemKind: "task",
      preparationKind: "last_minute",
      bag: "last_minute",
      bulk: "medium",
    },
  ),
  templateItem("general-last-phone", "手机", "last_minute", "must", "grab_before_leaving", {
    note: "出门前拿上常用手机，确认电量充足，并把医院和紧急联系人置顶。",
    removable: false,
    packTier: "core",
    itemKind: "task",
    preparationKind: "last_minute",
    bag: "last_minute",
    bulk: "small",
  }),
  templateItem("general-last-charger", "充电器", "last_minute", "must", "grab_before_leaving", {
    note: "出门前把与手机接口匹配的充电头和充电线装入随身包。",
    removable: false,
    packTier: "core",
    itemKind: "task",
    preparationKind: "last_minute",
    bag: "last_minute",
    bulk: "small",
  }),
  templateItem(
    "general-last-mom-bag",
    "妈妈包放到固定位置",
    "last_minute",
    "must",
    "grab_before_leaving",
    {
      note: "出门前从固定位置拿上妈妈包，拉好拉链并按清单快速复核证件和常用物品。",
      removable: false,
      packTier: "core",
      itemKind: "task",
      preparationKind: "last_minute",
      bag: "last_minute",
      bulk: "large",
    },
  ),
  templateItem(
    "general-last-baby-bag",
    "宝宝包放到固定位置",
    "last_minute",
    "must",
    "grab_before_leaving",
    {
      note: "出门前从固定位置拿上宝宝包，确认出院衣物、尿不湿和包被已经装好。",
      removable: false,
      packTier: "core",
      itemKind: "task",
      preparationKind: "last_minute",
      bag: "last_minute",
      bulk: "large",
    },
  ),
  templateItem(
    "general-last-medicine-list",
    "常用药清单 / 医生确认用药",
    "last_minute",
    "must",
    "grab_before_leaving",
    {
      note: "出门前带上已整理的常用药清单；如有长期用药，携带和使用方式提前向医生或护士确认。",
      removable: false,
      packTier: "core",
      itemKind: "task",
      preparationKind: "last_minute",
      bag: "last_minute",
      bulk: "small",
    },
  ),
  templateItem(
    "general-last-glasses",
    "眼镜 / 隐形眼镜",
    "last_minute",
    "must",
    "grab_before_leaving",
    {
      note: "出门前拿上框架眼镜和眼镜盒；隐形眼镜及护理用品按个人需要一并带走。",
      removable: false,
      packTier: "core",
      itemKind: "task",
      preparationKind: "last_minute",
      bag: "last_minute",
      bulk: "small",
    },
  ),
  templateItem(
    "general-last-payment",
    "支付 / 押金确认",
    "last_minute",
    "must",
    "grab_before_leaving",
    {
      note: "出门前确认可用的支付方式、额度和押金信息，需要实体银行卡或现金时放入证件包。",
      removable: false,
      packTier: "core",
      itemKind: "task",
      preparationKind: "task",
      bag: "none",
      bulk: "small",
    },
  ),
  templateItem(
    "general-last-car-seat",
    "安全座椅确认",
    "last_minute",
    "must",
    "grab_before_leaving",
    {
      note: "出门前确认使用新生儿适用的后向安全座椅，并按说明书检查安装方向、固定状态和卡扣路径，发现松动先重新调整。",
      removable: false,
      packTier: "core",
      itemKind: "task",
      preparationKind: "install_or_place",
      bag: "car",
      bulk: "large",
    },
  ),
];
