...
const pushRateLimiter = createRateLimiter(30, 60_000);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!isDadKitImportData(payload) || payload.version !== 11) {
      return syncError("同步数据格式无效，请更新 DadKit 后重试。", 400);
    }
    const snapshot = await pushSpace(credential.token, payload);
    ...
    return syncJson(snapshot, 200, {
      etag: createSyncEtag(snapshot.version),
