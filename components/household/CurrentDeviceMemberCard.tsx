...
export function CurrentDeviceMemberCard({ household, compact = false }: { household: HouseholdPortableData; compact?: boolean }) {
  ...
  return <section className={compact ? "grid gap-2" : "grid gap-3 rounded-card border border-border bg-card p-4"}><div><Label htmlFor={compact ? "baby-current-device-member" : "current-device-member"}>这台设备是谁在使用</Label><p className="mt-1 text-xs leading-5 text-muted-foreground">只保存在当前设备，不进入备份、WebDAV 或家庭同步。</p></div><Select value={currentMemberId ?? "none"} onValueChange={(value) => setCurrentMemberId(value === "none" ? null : value)}><SelectTrigger id={compact ? "baby-current-device-member" : "current-device-member"}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="none">暂不设置</SelectItem>{members.map((member) => <SelectItem key={member.id} value={member.id}>{member.displayName.value}{member.relationshipLabel.value ? ` · ${member.relationshipLabel.value}` : ""}</SelectItem>)}</SelectContent></Select></section>;
}
