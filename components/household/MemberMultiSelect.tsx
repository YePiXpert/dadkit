...
export function MemberMultiSelect({
  ...
  return (
    <div aria-label="负责人多选" className="grid gap-2" id={id} role="group">
      {active.map((member) => (
        <label className={cn("flex min-h-11 cursor-pointer items-center gap-3 rounded-xl border border-border px-3 py-2", selectedIds.includes(member.id) && "border-primary/50 bg-secondary")} key={member.id}>
          ...
          {member.relationshipLabel.value ? <span className="text-xs text-muted-foreground">{member.relationshipLabel.value}</span> : null}
        </label>
      ))}
      {historicalIds.map((memberId) => (
        <label className="flex min-h-11 cursor-pointer items-center gap-3 rounded-xl border border-dashed border-border px-3 py-2" key={memberId}>
          ...
          <span className="text-xs text-muted-foreground">历史负责人</span>
        </label>
      ))}
    </div>
  );
