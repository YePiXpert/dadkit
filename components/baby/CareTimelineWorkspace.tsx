...
export function CareTimelineWorkspace() {
  ...
  const visible = useMemo(() => {
    ...
    return [...map.values()].filter((event) => event.deletedAt === null && event.updatedAt > careClearedAt && (filter === "all" || event.type === filter) && (!normalizedQuery || event.note.toLocaleLowerCase().includes(normalizedQuery)));
