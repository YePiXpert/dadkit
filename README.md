...
DadKit 是一个给孕晚期家庭使用的本地优先待产准备 PWA。全站视觉为「精致现代简约风」：微粉暖白底、珊瑚粉主色、统一卡片/行/图标块规范（见 `docs/design/design-system.md`），界面不使用插画，视觉依靠渐变 hero 与分色图标 tile。待产包模板内置 117 个条目，每项配有物品级图标、数量参考和操作说明。功能上继续把准备方案收束成四根柱子：医院确认、核心待产包、临出门沟通卡和产后提醒，让“现在该做什么、还差什么、能发给谁看”变得更清楚。
...
- 🚗 **临出门模式**：把证件、手机、充电器、妈妈包、宝宝包和安全座椅集中到出发前逐项核对。
- ⏱️ **宫缩记录**：记录开始、结束、持续、间隔和备注，可导出给医生参考。
- 📝 **临出门沟通卡**：整理紧急联系人、陪产人、过敏、用药和沟通偏好。
...
- `/settings`：资料入口、本地快照、JSON 备份、WebDAV 备份和版本信息。
