...
- **数据由用户掌控**：JSON、本地恢复快照和 WebDAV 都只围绕清单备份。
- **纯 PWA**：浏览器安装、离线缓存和本地存储；不再包含 Android、iOS 或 Capacitor 工程。

## 主要功能

- 通用待产包清单，按证件、妈妈、宝宝、月子妈妈、宝宝家中囤货、陪产人、返家准备和临出门分组。
- 每个实物条目提供数量参考、用途或规格说明和彩色语义图标。
- 奶油色背景、珊瑚色行动强调和全新的奶瓶清单 PWA 图标，更适合长时间逐项核对。
- 自定义新增、编辑、标记不需要和恢复物品。
- 可为物品拍照或从相册上传，压缩后仅存当前设备，方便家人辨认实物。
- 数据默认仅保存在当前浏览器，可复制或导入 JSON、使用本地恢复快照，或手动配置 WebDAV 备份。
- 可安装到手机桌面，并缓存核心页面供基础离线使用。

DadKit 不提供医疗判断、医院官方规则或商品链接；物品备注中的品牌仅为常见示例，不代表推荐或合作。医院要求请以最近一次产检、入院须知和医院通知为准。
...
- `/`：待产包清单，也是 PWA 启动页。
- `/settings`：JSON、WebDAV、本地快照和清单恢复。
...
DadKit 不需要账号或后端数据库。当前浏览器数据全部使用独立的 `dadkit:v3:*` 命名空间；这是全新数据模型，不读取或迁移旧版数据，也不会在清理当前数据时删除旧命名空间。
...
JSON、本地恢复快照和 WebDAV 使用同一份清单备份数据。它们都不包含物品照片，也不包含 WebDAV 地址、用户名、路径、同步状态或密码等连接配置。

WebDAV 密码默认只放在 `sessionStorage`；只有用户主动选择“记住密码在本设备”时才写入 `localStorage`。连接配置和凭据不会写入 JSON、恢复快照或远端备份文件。

破坏性操作前会自动创建本地恢复快照，最多保留最近 5 份。首页的“恢复通用清单”只重置清单进度、自定义物品和隐藏项；“数据”页的“清空并重新开始”还会清除 WebDAV 配置和本机物品照片。操作前生成的恢复快照会保留，但它只能找回清单数据。
