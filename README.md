...
项目第一版定位为本地优先、手机端优先、适合 PWA 安装和开源发布。它不需要登录，不使用后端数据库，不接入支付、电商导购或真实联网 API。用户资料、清单状态和自定义内容都保存在浏览器本地。
...
第一版默认少而准：证件、妈妈少数关键物品、宝宝出院与基础备用、爸爸负责事项、医院待确认问题优先显示。充电宝、毛巾、宝宝指甲剪等低频或个人习惯项默认不进入精简模式，但保留在完整模式。
