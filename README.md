...
- 家庭分工与采购，按物品记录多个自定义负责人、完成期限、预计/实际总价、购买渠道和实际存放位置
...
- 统一设计系统：全站一致的状态徽章、骨架屏、危险操作区与表单控件，宝宝快速记录图标清晰可区分，家庭分工首屏更紧凑
...
家庭显示名称、成员名称、关系、多人负责人以及事件记录人会进入 JSON、IndexedDB 安全快照、WebDAV 和可选家庭同步；“当前设备使用者”只保存在当前设备，不作为独立设置进入任何备份或同步。较旧的数据格式仍可按其合法表示导入或同步，推送时会保留这些格式不认识的新字段。本次同步服务升级没有改变用户可移植数据格式。
...
1. 3.4.4 及以后版本会在应用内检查更新；「我的 → 关于 DadKit」会显示当前版本，并可随时手动检查更新（不受后台检查的 6 小时间隔限制）。
2. 点「下载更新」后可查看进度，APK 完成 SHA-256 校验后会直接打开 Android 系统安装界面。
3. 第一次使用应用内更新时，Android 可能要求允许 DadKit「安装未知应用」；授权后回到 DadKit 再点一次下载即可。
...
本次版本的详细变更与验收结果见 [DadKit 3.4.11 发布说明](docs/release-v3.4.11.md)。
