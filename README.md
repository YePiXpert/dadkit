...
- 统一设计系统：手机端五栏底部导航（首页/清单/宝宝/工具/我的），全站一致的状态徽章、骨架屏、危险操作区与表单控件，宝宝快速记录图标清晰可区分
...
家庭显示名称、成员名称、关系以及事件记录人会进入 JSON、IndexedDB 安全快照、WebDAV 和可选家庭同步；“当前设备使用者”只保存在当前设备，不作为独立设置进入任何备份或同步。当前可移植数据格式为 v11，不再写入已下线的家庭分工和医院信息字段；较旧的 v6-v10 数据仍可导入或同步，其中的旧医院信息会被安全忽略，旧分工字段也不会恢复。
