...
describe("v5/v6/v7/v8 family sync compatibility", () => {
  it("returns legal version projections without changing canonical data", async () => {
    ...
    const v8Data = (await pullSpace(v8.token, 8))?.data as DadKitExportData;
    ...
    expect(v7.planning.items.bag.assignee.value).toBe("dad");
  ...
  it("preserves unsupported canonical fields after v5, v6 and v7 pushes", async () => {
    ...
    const final = (await pullSpace(v8.token, 8))?.data as DadKitExportData;
    expect(final.checklist.some((item) => item.id === "from-v5")).toBe(true);
    expect(final.hospital.fields.address.value).toBe("健康路 2 号");
    expect(final.planning.items.bag.actualPriceFen.value).toBe(8_800);
  ...
  it("merges offline events and prevents deletion or global-clear resurrection", async () => {
    ...
    let final = (await pullSpace(v8.token, 8))?.data as DadKitExportData;
    ...
    await pushSpace(v8.token, first, 8);
    final = (await pullSpace(v8.token, 8))?.data as DadKitExportData;
  ...
  it("upgrades a stored v7 space to canonical v8 without inventing baby timestamps", async () => {
    ...
    const projected = (await pullSpace(v8.token, 8))?.data as DadKitExportData;
