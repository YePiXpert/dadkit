...
describe("release endpoints and product surface", () => {
  it("ships checklist-and-backup PWA metadata", () => {
    ...
    expect(packageJson.version).toBe("3.4.8");
  ...
  it("ships the full PWA surface inside the Android APK", () => {
    ...
    expect(activity).toContain("appVersionCode=21");
  ...
  it("keeps the Android tag release strict and verifies the signed APK", () => {
    ...
    expect(workflow).toContain('test "$GITHUB_REF_NAME" = "v3.4.8"');
  ...
  it("installs the entry shell, then keeps all core routes in the background cache list", () => {
    ...
    expect(sw).toContain('const CACHE_NAME = "dadkit-v3.4.8-pwa-r1"');
