...
describe("release endpoints and product surface", () => {
  it("ships checklist-and-backup PWA metadata", () => {
    ...
    expect(packageJson.version).toBe("3.3.1");
  ...
  it("ships a bundled Android WebView shell without TWA dependencies", () => {
    ...
    expect(activity).toContain("source=apk&appVersionCode=12");
    expect(activity).toContain("DadKitAndroid/12");
  ...
  it("keeps the Android tag release strict and verifies the signed APK", () => {
    ...
    expect(workflow).toContain('test "$GITHUB_REF_NAME" = "v3.3.1"');
  ...
  it("pre-caches the minimal offline entry routes during install", () => {
    ...
    expect(sw).toContain('const CACHE_NAME = "dadkit-v3.3.1-pwa-r1"');
