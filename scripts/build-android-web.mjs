...
try {
  ...
  // Keep the tracked root directory in place. Recreating it can intermittently
  // fail on Windows when Gradle/antivirus still has a handle on its parent.
  await mkdir(androidAssets, { recursive: true });
  for (const entry of await readdir(androidAssets)) {
    if (entry === ".gitkeep") continue;
    await rm(path.join(androidAssets, entry), { force: true, recursive: true });
  }
  for (const entry of await readdir(path.join(staging, "out"))) {
    await cp(
      path.join(staging, "out", entry),
      path.join(androidAssets, entry),
      { recursive: true },
    );
  }
